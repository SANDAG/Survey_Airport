...
### 1. `/notebooks/00-pre-process-raw-data.ipynb`
...
A dictionary is used in this script to rename variables. See `/data/processed/revised_names.csv`. This moves the data from the variable names used by the field team, which are useful for keeping track of question sequencing, and the variable names we want in the end data product. The output of this step is a clean survey file, i.e., `/data/interim/survey_data_clean.csv`, which has revised names and some additional useful identifiers and variables.

### 2. `/notebooks/01-pass-clean-data-through-data-model.ipynb`
This notebook starts with the clean dataset generated by step 1. It attempts to validate the data using the data model, including checking if all the fields are present and have allowed values (or combination of values).
...
This code first determines whether or not the relevant record is an `Employee` or an `ArrivingPassengerResident`, `ArrivingPassengerVisitor`, `DepartingPassengerResident` or a `DepartingPassengerVisitor` (all of which are based on fields `marketsegment` and `passenger_segment_label` of `Respondent` data model class) and then instantiates the relevant data model class. This instantiation does the validation. If the validation is unsuccessful, these "failed" records are stored in a separate data frame.
...
### 3. `/notebooks/02-survey-expansion.Rmd` 
...
### 4. `/notebooks/03-merge-weights-to-data.ipynb`
This notebook attaches four weight fields in the `data_model_output.csv` file generated in step 2 using the `weights_only_files` generated in step 3. This step should be run only after generating/updating all weights_only files using all the controls in `02-survey-expansion.Rmd`.

### 5. `notebooks/04-filter-data-model-complete-records.ipynb`

The `data_model_output.csv` is a fairly large file, and can be overwhelming to look at. This notebook creates a cleaner version, by eliminating invalid and incomplete records, and consolidating a few columns. The resulting file from this step is `/data/processed/atc_travel_survey_final_data.csv`, which is the cleanest version of the survey data. 

### 6. `/notebooks/05-create-variable-summaries.ipynb`
...
### 7. `/notebooks/06-compare-pilot-and-original.ipynb`
...
### 8. `/notebooks/07-mode-summaries-by-date.ipynb`
...
### 9. `/notebooks/08-create-zonal-distribution-summaries.ipynb`
...
### 10. `/notebooks/09-flight-time-of-day-comparisons.ipynb`
...
### 11. `/notebooks/10-make-excel-data-dictionary-from-data-model.ipynb`
...
### 12. `/notebooks/11-make-word-document-from-excel-data-dictionary.ipynb`
...
### 13. `/notebooks/12-make-word-document-from-excel-questionnaire.ipynb`
...
### 14. `notebooks/13-compare-thanksgiving-week-records.ipynb`
...
### 15. `notebooks/14-make-origin-destination-map`
...
### 16. `notebooks/15-compare-sas-and-intercept-data.ipynb`
...
### 17. `notebooks/16-calculate-walking-distance.ipynb`
...
### 18. `notebooks/17-business-vs-non-business-logic.csv`
...
### 19. `notebooks/18-compare_income_terminals.ipynb`
...
### 20. `notebooks/make-tall-data-for-select-tableau-summaries.ipynb`
...
### 21. `80-make-line-strings-from-gtfs-points.ipynb`
This notebook is used to make line strings (transit routes) from GTFS points, to visualize them in Tableau.

### 22. `81-tag-points-close-to-routes.ipynb`
This notebook is used to tag closest transit routes to trip origins, also used for visualization in the Tableau dashboard. 

### 23. `82-summaries-for-final-report.ipynb` and `83-create-data-summaries-for-ppt.ipynb`
Exploratory notebooks used to create summaries for the final report as well as the presentation, as required. 
